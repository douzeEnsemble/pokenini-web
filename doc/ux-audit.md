...
- [x] [basse] Coquille : "Je choisi celui-là" → "Je choisis celui-là"
...
- [x] [basse] Deux styles de barre de progression sans légende sur les cartes de dex (pleine segmentée vs hachurée)
    ...
    ⚠️ Constat corrigé avant de traiter : revérifié en direct (capture fraîche + dump DOM) — aucune carte d'album n'utilise de barre hachurée, toutes utilisent `catchStateBar` (pleine segmentée), y compris "Home Pokemon Go" à 100%. Le postulat initial (deux styles sur les cartes d'album) était une erreur de lecture. L'utilisateur a précisé que les hachures qu'il avait en tête sont sur les pages Élection (liste de dex à voter, page de vote), via `roundBar` (`progress-bar-striped`) — vérifié, bien réel là.
    **Traité** (sur le bon périmètre) : en revérifiant `roundBar` sur `/fr/election/dex`, la carte "Home" (0 tour voté) affichait une barre grise totalement vide, sans aucun "0%" — le `<div class="progress-bar" style="width:0%">0%</div>` original piège son propre texte dans une boîte large de 0px, qui ne s'affiche donc jamais. Corrigé dans `templates/common/_progress_bar_macros.html.twig` : le libellé "{{ progress }}%" sort du `.progress-bar` coloré (contraint en largeur) vers un `<span class="round-bar-label">` positionné en overlay centré sur le conteneur `.progress` externe — visible à 0% comme à 100%. Ajouté `.round-bar-label` dans `public/css/base.css` (texte noir + halo blanc en `text-shadow`, lisible aussi bien sur la piste grise que sur une barre colorée saturée). `ElectionDexListTest` mis à jour (le texte "31%" est désormais cherché sur `.round-bar-label`, plus sur `.progress-bar`). Vérifié : 40 tests / 828 assertions verts sur `tests/src/Integration/Controller/Election/`, rendu confirmé en navigateur réel (la carte "Home" affiche maintenant "0%" au lieu d'une piste vide).
