...
- Each task is implemented and committed on the pre-created `feature/credits-by-pokemon` branch of the repo it touches (`pokenini-api`, `pokenini-back`, or `pokenini-web` — already branched off `main`, isolated from it). Commit at the end of each task, one commit per task (matches the subagent-driven-development execution flow: task review diffs the task's commit range). **Do not push, merge, or open a PR** — that decision is the user's, made explicitly at the end via `finishing-a-development-branch`.
