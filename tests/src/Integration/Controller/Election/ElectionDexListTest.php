...
final class ElectionDexListTest extends WebTestCase
{
    ...
    public function testIndex(): void
    {
        ...
        $this->assertCountFilter($crawler, 6, '.dex-item .card-title');
        $this->assertCountFilter($crawler, 6, '.dex-item .card-title a');
        $this->assertCountFilter($crawler, 9, '.dex-item .badge');
        $this->assertCountFilter($crawler, 6, '.dex-item p.small');

        $this->assertSame('832 Pokémons', $crawler->filter('.dex-item .badge')->eq(0)->text());
        $this->assertSame('982 Pokémons', $crawler->filter('.dex-item .badge')->eq(1)->text());

        $this->assertSame('/fr/election/swordshield', $crawler->filter('.dex-item .card-title a')->eq(0)->attr('href'));
        $this->assertSame('/fr/election/homeshiny', $crawler->filter('.dex-item .card-title a')->eq(2)->attr('href'));
    }
}
